"""
CLUSTER ANALYSIS - THESIS VERSION
=================================
Reproduces the k-means solution reported in the dissertation, Chapter 4.3
(Sxima 18): four segments of roughly n = 67 / 81 / 182 / 186.

This is a copy of `../cluster analysis.py` with ONE change: the constructs are
NOT z-scored before clustering. See the comment inside standardize_data().

The script in the parent folder z-scores first and produces a different -
valid, but less informative - solution (n = 243 / 80 / 22 / 171). Both are
kept in this repository deliberately; see the README section
...
def standardize_data(X):
    ...
    print("\n" + "=" * 70)
    print("STEP 2: NO STANDARDISATION - RAW CONSTRUCT MEANS (THESIS VERSION)")
    print("=" * 70)

    # ---- THESIS VERSION: no z-scoring. Cluster on the raw 1-5 means. ----
    # This is the ONLY change from `../cluster analysis.py`.
    #
    # The twelve constructs do not have equal variance in the raw 1-5 space
    # (SD ranges from 0.83 for PE to 1.25 for RC). Trust in Technology
    # (SD = 1.19) and Regulatory & Compliance Risks (SD = 1.25) genuinely vary
    # more across respondents than the core UTAUT2 constructs. k-means weights
    # each dimension by its variance, so in raw space those two drive the
    # partition and the result is a trust-differentiated typology.
    #
    # Z-scoring forces every construct to contribute equally, erasing that
    # variance advantage: Trust drops from the STRONGEST separator between
    # clusters (spread 3.06 on the 1-5 scale) to the WEAKEST (2.13), and the
    # solution collapses into a general high/medium/low agreement gradient.
    #
    # Both are defensible. The raw-space solution is the one reported in the
    # dissertation and it also separates better (silhouette 0.271 vs 0.228).
    X_scaled = X.values
    X_scaled_df = pd.DataFrame(X_scaled, columns=X.columns, index=X.index)

    print("Clustering on RAW construct means (no standardisation).")
    print("\nConstruct standard deviations (these drive the k-means solution):")
    print(X.std().sort_values(ascending=False).round(3).to_string())
    print("\nRaw data statistics:")
    print(X_scaled_df.describe().round(2))

    return X_scaled, X_scaled_df, None
